...
### 2026-05-27 第四轮：剩余功能 + E2E测试
- [x] Feature 3.2：人脸聚类识别 + 人名标注
  - AI人脸检测（z-ai-web-dev-sdk视觉分析）
  - 32维人脸特征向量生成
  - Union-Find层次聚类算法（余弦相似度>0.75自动分组）
  - 人脸分组管理UI（FaceGroups + FaceGroupPhotos组件）
  - 侧边栏新增「人脸识别」导航入口
  - 数据库新增FaceGroup、FaceInstance两张表
- [x] Feature 3.3：人名关键词搜索照片
  - 搜索API集成人脸分组名匹配
  - 搜索结果展示「人脸匹配」标签
- [x] Feature 3.5：语义搜索（向量检索）
  - AI生成64维文本嵌入向量
  - SQLite存储嵌入（FileEmbedding表）
  - 混合搜索模式：0.4*关键词 + 0.6*语义
  - 搜索组件新增三模式切换（混合/智能/关键词）
  - 批量嵌入生成API（/api/embeddings/generate）
- [x] E2E自动化测试（Playwright）
  - 32个端到端测试用例
