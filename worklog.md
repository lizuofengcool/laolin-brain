...
## 2026-07-09 22:00 自动迭代

### 第一百四十一轮（fix faces/groups/[id] 跨租户越权 + 回归测试）

**背景**：上轮 worklog「下一轮候选」列的是测试清理/TODO 桩/i18n/支付 SDK 真接入等优先级 2-3 项。但本轮启动时按任务清单优先级 1（安全/逻辑问题 > 测试清理）重新审计 priority-1 #3「`src/app/api/files/route.ts` 等路由绕过 TenantDb 直接按 userId 过滤」的残留面：用 search subagent 全量审计 116 个 `src/app/api/**/route.ts`，发现 faces 模块的两处路由仍按「`tenantId` 从 auth 解构但从未使用、仅按 `userId`+`id` 校验」的旧范式——多租户用户知道 groupId 即可枚举/重命名/删除他租户人脸分组及其照片，属真实跨租户越权读写 bug。按优先级 1 优先原则本轮取此为目标，闭合 priority-1 #3 真正残留项。

前置确认：
- 沙箱再次重置无 laolin-brain 目录，`git clone` 自 Gitee origin 后补加 github remote，`git config user.email/name` 已设。
- 双端 fetch 后本地 / origin/main / github/main 三方对齐于 `90ac945`（第一百四十轮 worklog commit），无远端更新、无遗留未提交改动、工作树干净，直接开始新开发。
- priority-1 原 5 项审计结论：#1 tenant-db raw 后门、#2 payment RSA2 占位、#4 sync-engine keep_both、#5 api-auth 测试不符 均已在前若干轮闭合；#3「files/route.ts 等」经全量审计确认 files 路由已修，真正残留仅在 faces 模块，本轮闭合。

**fix — `src/app/api/faces/groups/[id]/route.ts`（PUT/DELETE）+ `src/app/api/faces/groups/[id]/photos/route.ts`（GET）**：

参照 `folders/[id]/route.ts` 的 `createTenantDb(tenantId)` 标准范式，三段查询全部改走 tenantDb 访问器（自动注入 tenantId）：

- **route.ts PUT**：`createTenantDb(tenantId)` 后 `tenantDb.faceGroup.findFirst({ where: { id } })` 过滤；返回 null 或 `userId !== auth.userId` → 404（不暴露存在性）；通过后保留 `db.faceGroup.update({ where: { id } })` 取回更新记录——因 `TenantDb.faceGroup.update` 走 `updateMany` 仅返回 count，无法回传完整记录给响应。
- **route.ts DELETE**：同 `findFirst` 校验后走 `tenantDb.faceGroup.deleteMany({ where: { id } })`（cascade 删除 face instances）。
- **photos/route.ts GET**：`faceGroup.findFirst` / `faceInstance.findMany`（take 5000 防 unbounded read）/ `file.findMany` 三段全部走 tenantDb；fileId 去重计算 total；分页 skip/take；tags 经 `safeJsonParseArray` 解析。

注释中标注原 bug 形态与 TenantDb 修复点，便于后续审计回溯。

**test — `src/__tests__/api/faces-groups-id-route.test.ts`（13 用例）+ `src/__tests__/api/faces-groups-id-photos-route.test.ts`（10 用例）**：

复用第三十轮 cloud-sync-config-route 的 `vi.hoisted` 共享 `MockNextResponse` 范式（使路由 `auth instanceof NextResponse` 命中），mock 边界仅 `next/server` / `@/lib/api-auth` / `@/lib/db`，`safeJsonParseArray` 为纯函数加载真实实现。

- **id-route（13）**：未认证 401；name 校验（空/非字符串/超 100）；findFirst 返回 null（租户隔离 → 404 不触达 update/deleteMany）；userId 不匹配 404；成功路径（`createTenantDb` 收到 tenantId、findFirst/update/deleteMany 调用参数、回传结构）；update 抛错 500；deleteMany 抛错 500。
- **id-photos-route（10）**：未认证 401；分页校验（page=abc/0、limit=0/200）；findFirst null 租户隔离 404；userId 不匹配 404；成功路径（三段查询经 tenantDb、fileId 去重 total=2、tags 经 safeJsonParseArray 解析）；分页 page=2&limit=1；file.findMany 抛错 500。

mock 策略要点：`beforeEach` 中 `vi.clearAllMocks` 会清掉 `vi.hoisted` 工厂的 `mockCreateTenantDb.mockReturnValue`，故需在 `beforeEach` 重建（与 faces-groups-route 范式一致）。stderr 中 `console.error: Error: db down` 为「抛错 → 500」用例故意触发，测试本身通过。

### 验证

- `npx vitest run src/__tests__/api/faces-groups-id-route.test.ts src/__tests__/api/faces-groups-id-photos-route.test.ts`：**23/23 通过**。
- `npx tsc --noEmit`：**0 错误**。
- `npx vitest run` 全量 **4918/4918 通过**（174 文件），零回归。测试数较上轮 4895 → 4918（+23），文件数 172 → 174（+2），精确匹配本轮新增。

环境备注：沙箱无 node_modules，`npm ci`（975 包）+ `npx prisma generate` 后跑测试；未触碰 lockfile。

### 改动量

4 文件，+591/-14：
- `src/app/api/faces/groups/[id]/route.ts`（fix，PUT/DELETE 改走 TenantDb，+11/-7）
- `src/app/api/faces/groups/[id]/photos/route.ts`（fix，GET 三段查询改走 TenantDb，+15/-7）
- `src/__tests__/api/faces-groups-id-route.test.ts`（test，新增，13 用例，+287/-0）
- `src/__tests__/api/faces-groups-id-photos-route.test.ts`（test，新增，10 用例，+278/-0）

2 commit（1 fix + 1 test），符合单轮 1-3 commit 约束。

### Commit

- `bcf3f2e` fix(faces): faces/groups/[id] 路由改走 TenantDb 闭合跨租户越权
- `3a7ba14` test(faces): 补 faces/groups/[id] 路由租户隔离回归测试 23 用例

### 推送

- origin (Gitee)：`90ac945..3a7ba14` 推送成功（2 commit）
- github (GitHub)：`90ac945..3a7ba14` 推送成功（2 commit）

### 下一轮候选

- **priority-1 #3 完全闭合**：faces/groups/[id] 及 photos 子路由已改走 TenantDb，全量 API 路由审计未见其他残留「tenantId 解构未用、仅按 userId 过滤」范式。priority-1 原 5 项全部闭合。
- **同型「inline 模拟」测试文件清理**（延续）：`tenant-security.test.ts`、`src/lib/utils/__tests__/security.test.ts` 等——可改用统一 mock 工厂减少重复。
- **剩余 TODO 桩集中区**（延续）：`src/lib/plugins/registry.ts`、`src/lib/saas/billing.service.ts:290`、`src/lib/monitoring/index.ts` sendToChannel 各渠道桩——可逐模块做 mock 边界单测补覆盖。
- **faces 模块其他路由审计**（可选）：faces/groups 顶层 GET 已有测试，但 faces/instances、faces/recognize 等子路由的租户隔离可做一轮专项审计确认无残留。
- **支付 SDK 真接入**（可选，依赖外部 SDK）：alipay/wechat 下单/查询/退款链路未接 alipay-sdk / wechatpay-node-v3，属功能完整性缺口（非安全缺口）。

## 2026-07-13 16:00 自动迭代

### 第一百四十二轮（faces 模块剩余路由租户隔离审计 + merge 路由回归测试）

**背景**：上轮 worklog「下一轮候选」将「faces 模块其他路由审计」列为可选项（faces/instances、faces/recognize 等）。本轮取该候选为目标，对 faces 模块全部路由做一轮专项租户隔离审计，确认无残留「tenantId 解构未用、仅按 userId 过滤」范式；审计发现 detect/groups(顶层)/groups/merge/process-all 四个未测试路由均正确以 `userId+tenantId` 双键作用域（无越权 bug），遂对其中控制流最复杂、租户隔离面最显式的 `groups/merge` 路由补 handler 级回归测试，锁定审计结论防回归。规模 1 test commit，符合单轮 1-3 commit 约束，无生产代码改动，零回归风险。

前置确认：
- 沙箱再次重置无 laolin-brain 目录，`git clone` 自 Gitee origin 后补加 github remote，`git config user.email/name` 已设。
- 双端 fetch 后本地 / origin/main / github/main 三方对齐于 `d79eb28`（第一百四十一轮 worklog commit），无远端更新、无遗留未提交改动、工作树干净，直接开始新开发。
- priority-1 原 5 项审计结论：均在前若干轮闭合（上轮 worklog 已确认）。

**审计 — faces 模块全部路由租户隔离专项**：

逐文件审查 `src/app/api/faces/**/route.ts`（detect / groups 顶层 / groups/[id] / groups/[id]/photos / groups/merge / process-all），结论：

- **detect/route.ts**：`db.file.findUnique({ where: { id: fileId } })` 后校验 `file.userId !== userId || file.tenantId !== tenantId` → 403；后续 faceInstance/faceGroup 查询均以 `{ userId, tenantId }` 作用域；create 写入 tenantId。✅ 正确作用域（fetch-then-check 范式，校验在变更前）。
- **groups/route.ts（GET）**：`where: { tenantId, userId }` 双键。✅
- **groups/[id] + photos**：上轮已改走 TenantDb。✅
- **groups/merge/route.ts**：目标分组 `findUnique` 后校验 `userId !== auth.userId || tenantId !== auth.tenantId` → 404；源分组 `findMany({ where: { id: { in }, userId, tenantId } })` 双键过滤；事务内仅操作已验证的 targetGroupId / sourceGroup.id。✅ 正确作用域。
- **process-all/route.ts**：`db.file.findMany({ where: { userId, tenantId, ... } })` / `faceGroup.findMany({ where: { userId, tenantId } })` 双键；create 写入 tenantId。✅

全模块无残留越权范式，priority-1 #3 审计面在 faces 模块彻底闭合。

**test — `src/__tests__/api/faces-groups-merge-route.test.ts`（+413 行，18 用例）**：

复用 faces-groups-id-route.test.ts 的 `vi.hoisted` 共享 `MockNextResponse` 范式。merge 路由用 `db` 直接访问（非 createTenantDb），故 mock 边界为 `next/server` / `@/lib/api-auth` / `@/lib/db`（faceGroup.findUnique + findMany + `$transaction`）。`$transaction` mock 透传 `mockTx` 给回调（与真实 Prisma 行为一致）；`beforeEach` 中 `vi.clearAllMocks` 会清掉 hoisted 工厂的 `mockTransaction.mockImplementation`，故在 `beforeEach` 重建透传。mockTx 提供 faceInstance.updateMany/findMany + faceGroup.delete/update。

按控制流组织用例：

- **认证（1）**：未认证 → 401 透传，不触达 DB。
- **参数校验（6）**：sourceGroupIds 缺失/非数组/空数组 → 400；targetGroupId 缺失/非字符串 → 400；targetGroupId ∈ sourceGroupIds → 400。
- **租户隔离（4）**：目标分组 findUnique 返回 null / userId 不匹配 / tenantId 不匹配 → 404，不触达 findMany/$transaction；源分组 findMany 返回数量不一致 → 404 含缺失 id，不触达 $transaction。锁定 findMany 以 `{ id:{in}, userId, tenantId }` 双键过滤的契约。
- **事务控制流（2）**：成功路径 updateMany({where:{groupId:src.id},data:{groupId:target}}) + delete({where:{id:src.id}})，响应字段透传；多源分组 totalMovedFaces 聚合 updateMany.count（2+5=7）。
- **名称继承（2）**：目标 name 为空且某源分组有 name → tx.faceGroup.update 写入源名称；目标已有 name → 不触发 update。
- **缩略图更新（2）**：合并后 max-count fileId 与原 thumbnail 不同 → update 写入新缩略图；相同 → 不更新。锁定 `tx.faceInstance.findMany({where:{groupId:target},select:{fileId:true}})` + Map 计数 + max-count 选取 + diff 判定的完整逻辑。
- **错误兜底（1）**：$transaction 抛错 → 500 '合并分组失败'。

mock 策略要点：缩略图测试通过控制 `mockInstanceFindMany` 返回值驱动 Map 计数与 max-count 选取（如 `[{fileId:"new-thumb"},{fileId:"new-thumb"},{fileId:"file-thumb"}]` → new-thumb count=2 > file-thumb count=1 → 选 new-thumb）。stderr 中 `Merge face groups error: Error: tx down` 为「$transaction 抛错 → 500」用例故意触发，测试本身通过。

### 验证

- `npx vitest run src/__tests__/api/faces-groups-merge-route.test.ts`：**18/18 通过**（首轮即绿）。
- `npx tsc --noEmit`（prisma generate 后）：**0 错误**（test 文件被 tsconfig exclude，mock db 类型与 Prisma 模型方法签名兼容）。
- `npx vitest run` 全量 **4936/4936 通过**（175 文件），零回归。测试数较上轮 4918 → 4936（+18），文件数 174 → 175（+1），精确匹配本轮新增。改动仅新增一个测试文件，未触碰任何生产代码，不可能影响其他测试。

环境备注：沙箱无 node_modules，lockfile 为 `package-lock.json`（npm），`npm ci` 安装（975 包，49s）后 `npx prisma generate`（v6.19.3 客户端），再跑测试；未触碰 lockfile，`git status` 仅本轮 1 个新增测试文件。

### 改动量

1 文件，+413/-0：
- `src/__tests__/api/faces-groups-merge-route.test.ts`（test，新增）— 18 用例覆盖 merge 路由全控制流（+413/-0）

1 dev commit（纯测试补覆盖 + 审计结论记录，无生产改动）。

### Commit

- `92fb25f` test(faces): 补 faces/groups/merge 路由租户隔离与事务回归测试 18 用例

### 推送

- origin (Gitee)：`d79eb28..` 推送（含本轮 test 1 commit + 本 worklog commit）
- github (GitHub)：同上

### 下一轮候选

- **faces 模块审计闭合**：detect / groups 顶层 / merge / process-all 四路由均确认正确租户作用域，merge 已补回归测试。剩余 detect / process-all 路由可补测试（detect 需 mock detectFaces AI 模块 + cosineSimilarity；process-all 含内存态后台处理 + 文件读取，mock 边界较重），优先级低于其他候选。
- **同型「inline 模拟」测试文件清理**（延续）：`tenant-security.test.ts`、`src/lib/utils/__tests__/security.test.ts` 等——可改用统一 mock 工厂减少重复。
