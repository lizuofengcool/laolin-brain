...
import type { FolderItem, StoreSet, StoreGet } from "./types";
import { getStorageAdapter } from "@/lib/storage/factory";
import { DB_VERSION } from "@/lib/storage/indexeddb";
import { useNotificationStore } from "@/stores/notification-store";
import { useActivityStore } from "@/stores/activity-store";

export function createFileSlice(set: StoreSet, get: StoreGet) {
  return {
    // ── Files State ──────────────────────────────────────────────────────
    files: [] as FileData[],
    ...
    updateFile: (id: string, data: Partial<FileData>) =>
      set((s) => ({
    ...
    batchToggleFavorite: async (ids: string[], value: boolean) => {
      const { user, storageMode } = get();
      if (!user) return;
      // Optimistic update
      for (const id of ids) {
    ...
    // ── File type filter ──────────────────────────────────────────────────
    fileTypeFilter: null as string | null,
    setFileTypeFilter: (filter: string | null) => set({ fileTypeFilter: filter }),
    ...
    addFolder: (folder: FolderItem) => set((s) => ({ folders: [...s.folders, folder] })),
    ...
    moveFileToFolder: async (fileId: string, folderId: string | null) => {
      ...
      try {
        const adapter = getStorageAdapter(storageMode);
        await adapter.updateFile(fileId, { folderId: folderId || null } as Partial<FileData>, user.id);
