...
describe('simpleHash', () => {
  ...
  it('matches the same algorithm used in BackupRestore.tsx', () => {
    // Verify determinism with a known input
    const result = simpleHash('hello');
    expect(typeof result).toBe('string');
    expect(result.length).toBeGreaterThan(0);
    // Ensure it matches the client-side simpleHash by computing twice
...
describe('verifyChecksum', () => {
  ...
  it('returns true when no checksum is provided (skip verification)', () => {
    expect(verifyChecksum(testData, undefined)).toBe(true);
    expect(verifyChecksum(testData, '')).toBe(true);
  });
