 ...
 *      `!skipDocAiDueToRateLimit` false → IIFE 不启动 → fetch 不触达、tenantDb.file.update 不触达、
 ...
 *   - rate-limit 不触达 AI fetch：image 用例 fetch 未调；doc 用例 fetch 未调 + tenantDb.file.update 未调
 *     （IIFE 未启动）。用 settleIife() flush microtask 链后断言 tenantDb.file.update 未触达（不可用
...
const {
  ...
  mockRawFileUpdate,  // tenantDb.file.update（doc AI summarize IIFE；rate-limit 用例不触达，负向）
...
vi.mock("@/lib/db", () => ({
  ...
  // file.update 经 createTenantDb().file.update（doc IIFE；rate-limit 用例不触达，负向），
 ...
 * 用于 doc rate-limit 负向用例：flush 后断言 tenantDb.file.update 未触达（不可用 vi.waitFor 轮询
...
describe("/api/files 主路由 POST — rate-limit 触发 aiSkipped:true（子轮②d）", () => {
  ...
  it("① image rate-limit：user-ratelimit 第 11 次 image 上传 → checkAiRateLimit false → aiSkipped=true → fetch 不触达；generateThumbnail 仍触达（在 rate-limit 检查之前）；文件存储 $transaction/file.create 仍完成；响应 aiSkipped:true、textContent:undefined（无 OCR）、tags:[]", async () => {
    ...
    // 负向：image 不在 docTypes → tenantDb.file.update（doc IIFE）本就不触达（与 rate-limit 无关）
    expect(mockRawFileUpdate).not.toHaveBeenCalled();
  });

  it("② doc rate-limit：user-ratelimit doc 上传 → 非 image 跳过 image 块 → line 372 checkAiRateLimit false → skipDocAiDueToRateLimit=true → aiSkipped=true → IIFE 不启动 → fetch 不触达、tenantDb.file.update 不触达；响应 aiSkipped:true、textContent=buffer.toString、tags:[]、无 summary", async () => {
    ...
    expect(mockFetch).not.toHaveBeenCalled();
    // flush IIFE microtask 链（即便 IIFE 未启动，flush 确保无 stray microtask）后断言 tenantDb.file.update 未触达
    await settleIife();
    expect(mockRawFileUpdate).not.toHaveBeenCalled();
