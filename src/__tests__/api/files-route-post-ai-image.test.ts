 ...
 *   - 图片不在 docTypes → tenantDb.file.update（doc AI summarize fire-and-forget）不触达（负向）。
 *
 ...
 *     file.create）+ file.update（tenantDb.file.update，图片不在 docTypes → 不触达，负向）；
...
const {
  ...
  mockRawFileUpdate,  // tenantDb.file.update（doc AI summarize；图片不在 docTypes → 不触达，负向）
...
vi.mock("@/lib/db", () => ({
  ...
  // file.update 经 createTenantDb().file.update（图片不在 docTypes → 不触达，负向），
...
describe("/api/files 主路由 POST — 图片 AI process-image fetch（子轮②c-image）", () => {
  ...
  it("① happy path：jpeg + 无 skipAi + process-image 返回 {ocrText, tags} → generateThumbnail + fetch(process-image) 阻塞 await → textContent=ocrText, tags=aiTags → 新建 $transaction(tx.file.create data 含 AI 产物) → 200；fetch URL/method/body/Authorization 透传；tenantDb.file.update 不触达（图片不在 docTypes）", async () => {
    ...
    // 负向：图片不在 docTypes → tenantDb.file.update（doc AI summarize fire-and-forget）不触达
