 ...
 * GET（密码验证已移除）：
 *   - 无密码分享 → 200 回包文件信息。
 *   - 密码保护分享 → 始终 403 passwordRequired，**即使带 ?password= query param 也忽略**
 *     （修复前 GET 接受 ?password=，密码会泄漏到 URL/访问日志/Referer/浏览器历史）。
 *   - 不存在 token → 404；已过期 → 410。
 *
 * POST 密码验证（未认证分支）+ 按 token 暴破防护：
 *   - 正确密码 → 200 回包文件信息 + clearSharePasswordLimit 被调用（清失败计数）。
...
function makeGetRequest(token: string, query = ""): { req: NextRequest; ctx: { params: Promise<{ id: string }> } } {
...
describe("/api/files/[id]/share POST 密码验证（未认证）+ 按 token 暴破防护", () => {
  ...
  it("正确密码 → 200 回包文件信息 + clearSharePasswordLimit 被调用", async () => {
    mockFileShareFindUnique.mockResolvedValue(makeShare({ password: hashPwd("s3cret") }));
    ...
    expect(res.status).toBe(200);
    expect((res.body as { fileName: string }).fileName).toBe("doc.pdf");
